...
{
    public partial class InputPengadaanBarangHabisPakaiForm : ComponentFactory.Krypton.Toolkit.KryptonForm
    {
        public PengadaanHabisPakai selectedPengadaan;
        public bool isDetailMode = false;

        AppDbContext db = new AppDbContext();
        private List<PermintaanHp> selectedPermintaanList = new List<PermintaanHp>();
        ...
        public InputPengadaanBarangHabisPakaiForm()
        {
            InitializeComponent();
        }

        private void InputPengadaanBarangHabisPakaiForm_Load(object sender, EventArgs e)
        {
            SetupComboBoxes();
            ...
            if (isDetailMode)
            {
                this.Text = "Detail Pengadaan Barang Habis Pakai";
        ...
        {
            int lastId = db.PengadaanHabisPakai.OrderByDescending(p => p.IdPengadaan)
                                               .Select(p => p.IdPengadaan)
                                               .FirstOrDefault();

            int nextId = lastId + 1;
            return $"PHP-{DateTime.Now.Year}-{nextId:D3}"; // PHP = Pengadaan Habis Pakai
        }

        private void LoadDaftarPermintaan()
        {
            var permintaanDisetujui = db.PermintaanHp.AsNoTracking()
                .Include(p => p.DetailPermintaanHp)
                .Where(p => p.StatusPersetujuan == "Disetujui")
                .Where(p => !p.PengadaanPermintaanHp.Any())
                .ToList();
            ...
            {
                Pilih = false,
                KodePermintaan = p.KodePermintaanHp,
        ...
        private void GenerateDetailBon()
        {
            ...
            var listKodeDipilih = selectedPermintaanList.Select(p => p.KodePermintaanHp).ToList();
            var allDetailsDb = db.DetailPermintaanHp
                                 .Where(d => listKodeDipilih.Contains(d.KodePermintaanHp))
                                 .ToList();

            var detailsWithInduk = allDetailsDb.Select(d => new
            {
                Detail = d,
                PermintaanInduk = selectedPermintaanList.FirstOrDefault(p => p.KodePermintaanHp == d.KodePermintaanHp)
            }).Where(x => x.PermintaanInduk != null).ToList();

            var grouped = detailsWithInduk
                .GroupBy(x => new { x.Detail.IdMasterBarang, x.PermintaanInduk.KodePermintaanHp, x.PermintaanInduk.IdJurusan })
                .Select(g => new
                {
                    IdMasterBarang = g.Key.IdMasterBarang,
                    KodePermintaan = g.Key.KodePermintaanHp,
            ...
            foreach (var item in grouped)
            {
                ...
                if (barang != null)
                {
                    string namaKat = (barang.IdKategori.HasValue && dictKategori.ContainsKey(barang.IdKategori.Value))
                                     ? dictKategori[barang.IdKategori.Value]
                                     : "N/A";
        ...
        private void LoadDataPengadaan()
        {
            ...
            var details = db.DetailPengadaanHp.AsNoTracking()
                .Where(d => d.IdPengadaanHp == selectedPengadaan.IdPengadaan)
                .ToList();
            ...
            listDetailBon = new BindingList<DetailPengadaanViewModel>(
                details.Select(d => {
                    ...
                    string namaBarang = barang != null ? barang.NamaBarang : "N/A";
                    string namaKategori = (barang != null && barang.IdKategori.HasValue && dictKategori.ContainsKey(barang.IdKategori.Value))
                                          ? dictKategori[barang.IdKategori.Value] : "N/A";
                    string namaPemasok = (d.IdPemasok.HasValue && dictPemasok.ContainsKey(d.IdPemasok.Value))
                                          ? dictPemasok[d.IdPemasok.Value] : "-";

                    return new DetailPengadaanViewModel
                    {
            ...
            var kodePermintaanList = db.PengadaanPermintaanHp
                .Where(pp => pp.IdPengadaanHp == selectedPengadaan.IdPengadaan)
                .Select(pp => pp.KodePermintaanHp)
                .ToList();

            var permintaanListRaw = db.PermintaanHp.AsNoTracking()
                .Include(p => p.DetailPermintaanHp)
                .Where(p => kodePermintaanList.Contains(p.KodePermintaanHp))
                .ToList();
        ...
        private void btnTerapkanPemasok_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Silakan pilih pemasok dari daftar terlebih dahulu.");
                return;
            }

            int idPemasokTerpilih = (int)cmbPemasok.SelectedValue;
            string namaPemasokTerpilih = cmbPemasok.Text;
            ...
            if (dgDetailBon.SelectedRows.Count > 0)
            {
                ...
                MessageBox.Show($"Pemasok diterapkan ke {dgDetailBon.SelectedRows.Count} barang terpilih.");
            }
            else
            {
                if (MessageBox.Show("Terapkan pemasok ini ke SEMUA barang di keranjang?", "Konfirmasi", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    foreach (var item in listDetailBon)
                    {
                        item.IdPemasok = idPemasokTerpilih;
                    ...
                    MessageBox.Show("Pemasok diterapkan ke semua barang.");
                }
            }
