...
{
    public partial class LaporanBangunanNonAktifUC : UserControl
    {
        AppDbContext db = new AppDbContext();
        PrintDocument printDoc = new PrintDocument();
        List<LaporanBangunanNonAktifViewModel> printData = new List<LaporanBangunanNonAktifViewModel>();
        int currentPrintIndex = 0;

        public class LaporanBangunanNonAktifViewModel
        {
        ...
        public LaporanBangunanNonAktifUC()
        {
            InitializeComponent();
            InitializeEvents();
        }
        ...
        {
            this.Load += LaporanBangunanNonAktifUC_Load;
        ...
        private void LaporanBangunanNonAktifUC_Load(object sender, EventArgs e)
        {
        ...
        private void BtnTampilkanData_Click(object sender, EventArgs e)
        {
            ...
            var query = db.VBangunanNonAktif.AsNoTracking()
                        .Where(v => v.TanggalNonaktif >= tglAwal && v.TanggalNonaktif <= tglAkhir);

            printData = query.ToList().Select(v => new LaporanBangunanNonAktifViewModel
            {
                IdBangunanNonAktif = v.IdBangunanNonAktif,
            ...
            if (dgv != null)
            {
                dgv.DataSource = new SortableBindingList<LaporanBangunanNonAktifViewModel>(printData);
        ...
        private void PrintDoc_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            g.DrawString("LAPORAN BANGUNAN / GEDUNG NON AKTIF", new Font("Arial", 12, FontStyle.Bold), Brushes.Black, new RectangleF(margin, yPos, pageWidth, 20), formatCenter);
            ...
            string[] headers = { "No", "Kode Bangunan", "Nama Bangunan", "Luas (m²)", "Tgl Nonaktif", "Penyebab", "Keterangan" };
            int[] colWidths = { 30, 100, 220, 70, 100, 220, 230 };
            ...
            while (currentPrintIndex < printData.Count)
            {
                ...
                string[] rowData = { (currentPrintIndex + 1).ToString(), item.KodeBangunan?.ToString() ?? "-", item.NamaBangunan, item.LuasBangunan?.ToString("N0") ?? "-", item.TanggalNonaktif.ToString("dd-MM-yyyy"), item.Penyebab, item.Keterangan };
                for (int i = 0; i < rowData.Length; i++)
                {
                    ...
                    StringFormat fmt = (i == 0 || i == 1 || i == 3 || i == 4) ? formatMidCenter : formatMidLeft;
        ...
        private void BtnExportToExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                sfd.Filter = "Excel Files (*.xlsx)|*.xlsx"; sfd.FileName = "Laporan_Bangunan_NonAktif_" + DateTime.Now.ToString("yyyyMMdd");
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        {
                            var ws = package.Workbook.Worksheets.Add("Bangunan Non Aktif");
                            ws.Cells["A1:G1"].Merge = true; ws.Cells["A1"].Value = "LAPORAN BANGUNAN / GEDUNG NON AKTIF";
                            ...
                            for (int i = 0; i < printData.Count; i++)
                            {
                                ...
                                ws.Cells[row, 1].Value = i + 1; ws.Cells[row, 2].Value = item.KodeBangunan;
                                ws.Cells[row, 3].Value = item.NamaBangunan; ws.Cells[row, 4].Value = item.LuasBangunan;
                                ws.Cells[row, 5].Value = item.TanggalNonaktif.ToString("yyyy-MM-dd");
                                ws.Cells[row, 6].Value = item.Penyebab; ws.Cells[row, 7].Value = item.Keterangan;
                                for (int col = 1; col <= 7; col++) ws.Cells[row, col].Style.Border.BorderAround(ExcelBorderStyle.Thin);
                                row++;
                            }
