    ...
    public partial class LaporanPeminjamanBarangUC : UserControl
    {
        ...
        private void BtnExportToExcel_Click(object sender, EventArgs e)
        {
            ...
            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                ...
                sfd.FileName = "Laporan_Peminjaman_Barang_" + DateTime.Now.ToString("yyyyMMdd");
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        {
                            var ws = package.Workbook.Worksheets.Add("Peminjaman");

                            ws.Cells["A1:J1"].Merge = true;
                            ws.Cells["A1"].Value = "LAPORAN PEMINJAMAN BARANG";
                            ws.Cells["A1"].Style.Font.Bold = true;
                            ws.Cells["A1"].Style.Font.Size = 14;
                            ws.Cells["A1"].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;

                            string[] headers = { "No", "No Peminjaman", "Tgl Pinjam", "Peminjam", "No Telepon", "Kode Inventaris", "Nama Barang", "Lama (Hari)", "Jatuh Tempo", "Status", "Keterangan" };
                            for (int i = 0; i < headers.Length; i++)
                            {
                            ...
                            for (int i = 0; i < printData.Count; i++)
                            {
                                ...
                                ws.Cells[row, 8].Value = item.LamaPinjamHari;
                                ws.Cells[row, 9].Value = item.TanggalJatuhTempo;
                                ws.Cells[row, 10].Value = item.StatusPeminjaman;
                                ws.Cells[row, 11].Value = item.Keterangan;
