    ...
    public partial class LaporanMutasiBarangUC : UserControl
    {
        ...
        private void BtnExportToExcel_Click(object sender, EventArgs e)
        {
            ...
            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                ...
                sfd.FileName = "Laporan_Mutasi_Barang_" + DateTime.Now.ToString("yyyyMMdd");
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        {
                            var ws = package.Workbook.Worksheets.Add("Mutasi");

                            ws.Cells["A1:G1"].Merge = true;
                            ws.Cells["A1"].Value = "LAPORAN MUTASI BARANG";
                            ws.Cells["A1"].Style.Font.Bold = true;
                            ws.Cells["A1"].Style.Font.Size = 14;
                            ws.Cells["A1"].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;

                            string[] headers = { "No", "Tanggal Mutasi", "Kode Inventaris", "Nama Barang", "Jurusan Asal", "Jurusan Tujuan", "Alasan Mutasi" };
                            for (int i = 0; i < headers.Length; i++)
                            {
                            ...
                            for (int i = 0; i < printData.Count; i++)
                            {
                                ...
                                ws.Cells[row, 2].Value = item.TanggalMutasi.ToString("yyyy-MM-dd");
                                ws.Cells[row, 3].Value = item.KodeInventaris;
                                ws.Cells[row, 4].Value = item.NamaBarang;
                                ws.Cells[row, 5].Value = item.JurusanAsal;
                                ws.Cells[row, 6].Value = item.JurusanTujuan;
                                ws.Cells[row, 7].Value = item.AlasanMutasi;
