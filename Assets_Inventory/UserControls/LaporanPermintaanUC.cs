...
{
    public partial class LaporanPermintaanUC : UserControl
    {
        AppDbContext db = new AppDbContext();
        PrintDocument printDoc = new PrintDocument();
        List<LaporanPermintaanViewModel> printData = new List<LaporanPermintaanViewModel>();
        int currentPrintIndex = 0;

        public class LaporanPermintaanViewModel
        {
        ...
        {
            this.Load += LaporanPermintaanUC_Load;
        ...
        private void LaporanPermintaanUC_Load(object sender, EventArgs e)
        {
            LoadComboBoxes();
        ...
        private void LoadComboBoxes()
        {
            ...
            if (cmbStatus != null)
            {
                ...
                cmbStatus.Items.AddRange(new object[] { "-- Semua Status --", "Menunggu", "Disetujui", "Ditolak" });
        ...
        private void BtnTampilkanData_Click(object sender, EventArgs e)
        {
            ...
            var query = db.VLaporanPermintaan.AsNoTracking()
                        .Where(v => v.TanggalPermintaan >= tglAwal && v.TanggalPermintaan <= tglAkhir);

            if (!string.IsNullOrEmpty(statusFilter))
            {
                query = query.Where(v => v.StatusPersetujuan == statusFilter);
            }

            printData = query.ToList().Select(v => new LaporanPermintaanViewModel
            {
                KodePermintaan = v.KodePermintaan ?? "-",
                TanggalPermintaan = v.TanggalPermintaan,
                NamaJurusan = v.NamaJurusan ?? "-",
                NamaPeminta = v.NamaPeminta ?? "-",
                NamaBarang = v.NamaBarang ?? "-",
                JumlahDiminta = v.JumlahDiminta,
                AlasanKebutuhan = v.AlasanKebutuhan ?? "-",
                StatusPersetujuan = v.StatusPersetujuan ?? "-"
            }).OrderBy(x => x.TanggalPermintaan).ToList();
            ...
            if (dgv != null)
            {
                dgv.DataSource = new SortableBindingList<LaporanPermintaanViewModel>(printData);

                if (dgv.Columns["TanggalPermintaan"] != null)
                {
                    dgv.Columns["TanggalPermintaan"].DefaultCellStyle.Format = "dd MMM yyyy";
                }
            }
        ...
        private void PrintDoc_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            g.DrawString("LAPORAN PERMINTAAN BARANG", new Font("Arial", 12, FontStyle.Bold), Brushes.Black, new RectangleF(margin, yPos, pageWidth, 20), formatCenter);

            var dtAwal = this.Controls.Find("dtAwal", true).FirstOrDefault() as DateTimePicker;
            var dtAkhir = this.Controls.Find("dtAkhir", true).FirstOrDefault() as DateTimePicker;
            string tglStr = $"Periode: {dtAwal?.Value.ToString("dd MMM yyyy")} s.d {dtAkhir?.Value.ToString("dd MMM yyyy")}";
            ...
            string[] headers = { "No", "Kode Permintaan", "Tanggal", "Jurusan", "Peminta", "Nama Barang", "Jml", "Alasan", "Status" };
            int[] colWidths = { 25, 110, 85, 110, 120, 150, 40, 200, 80 };
            ...
            while (currentPrintIndex < printData.Count)
            {
                ...
                string[] rowData = {
                    ...
                    item.KodePermintaan,
                    item.TanggalPermintaan.ToString("dd-MM-yyyy"),
                    item.NamaJurusan,
                    item.NamaPeminta,
                    item.NamaBarang,
                    item.JumlahDiminta.ToString(),
                    item.AlasanKebutuhan,
                ...
                for (int i = 0; i < rowData.Length; i++)
                {
                    ...
                    StringFormat fmt = (i == 0 || i == 2 || i == 6 || i == 8) ? formatMidCenter : formatMidLeft;
        ...
        private void BtnExportToExcel_Click(object sender, EventArgs e)
        {
            ...
            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                ...
                sfd.FileName = "Laporan_Permintaan_" + DateTime.Now.ToString("yyyyMMdd");
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        {
                            var ws = package.Workbook.Worksheets.Add("Permintaan");

                            ws.Cells["A1:H1"].Merge = true;
                            ws.Cells["A1"].Value = "LAPORAN PERMINTAAN BARANG";
                            ws.Cells["A1"].Style.Font.Bold = true;
                            ws.Cells["A1"].Style.Font.Size = 14;
                            ws.Cells["A1"].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;

                            string[] headers = { "No", "Kode Permintaan", "Tanggal", "Jurusan", "Peminta", "Nama Barang", "Jumlah Diminta", "Alasan Kebutuhan", "Status Persetujuan" };
                            for (int i = 0; i < headers.Length; i++)
                            {
                            ...
                            for (int i = 0; i < printData.Count; i++)
                            {
                                ...
                                ws.Cells[row, 2].Value = item.KodePermintaan;
                                ws.Cells[row, 3].Value = item.TanggalPermintaan.ToString("yyyy-MM-dd");
                                ws.Cells[row, 4].Value = item.NamaJurusan;
                                ws.Cells[row, 5].Value = item.NamaPeminta;
                                ws.Cells[row, 6].Value = item.NamaBarang;
                                ws.Cells[row, 7].Value = item.JumlahDiminta;
                                ws.Cells[row, 8].Value = item.AlasanKebutuhan;
                                ws.Cells[row, 9].Value = item.StatusPersetujuan;
