...
{
    public partial class LaporanPengembalianBarangUC : UserControl
    {
        AppDbContext db = new AppDbContext();
        PrintDocument printDoc = new PrintDocument();
        List<LaporanPengembalianViewModel> printData = new List<LaporanPengembalianViewModel>();
        int currentPrintIndex = 0;

        public class LaporanPengembalianViewModel
        {
        ...
        public LaporanPengembalianBarangUC()
        {
            InitializeComponent();
            InitializeEvents();
        }
        ...
        {
            this.Load += LaporanPengembalianBarangUC_Load;

            // Form ini menggunakan nama kontrol generik (button1-4, dateTimePicker1-2, comboBox1, dataGridView1)
            if (this.Controls.Find("button4", true).FirstOrDefault() is Button btnTampil)
                btnTampil.Click += BtnTampilkanData_Click;

            if (this.Controls.Find("button2", true).FirstOrDefault() is Button btnPrev)
                btnPrev.Click += BtnPreview_Click;

            if (this.Controls.Find("button3", true).FirstOrDefault() is Button btnExport)
                btnExport.Click += BtnExportToExcel_Click;

            if (this.Controls.Find("button1", true).FirstOrDefault() is Button btnTutup)
        ...
        private void LaporanPengembalianBarangUC_Load(object sender, EventArgs e)
        {
            LoadComboBoxes();

            var dtAwal = this.Controls.Find("dateTimePicker2", true).FirstOrDefault() as DateTimePicker;
            var dtAkhir = this.Controls.Find("dateTimePicker1", true).FirstOrDefault() as DateTimePicker;
        ...
        {
            var comboBox1 = this.Controls.Find("comboBox1", true).FirstOrDefault() as ComboBox;
            ...
            if (comboBox1 != null)
            {
                ...
                comboBox1.DisplayMember = "NamaBarang";
                comboBox1.ValueMember = "IdMasterBarang";
            }
        }
        ...
        private void BtnTampilkanData_Click(object sender, EventArgs e)
        {
            ...
            var query = db.VLaporanPengembalian.AsNoTracking()
                        .Where(v => v.TanggalKembali >= tglAwal && v.TanggalKembali <= tglAkhir);

            printData = query.ToList().Select(v => new LaporanPengembalianViewModel
            {
                IdPengembalian = v.IdPengembalian,
                NomorPeminjaman = v.NomorPeminjaman ?? "-",
                NamaPeminjam = v.NamaPeminjam ?? "-",
                TanggalPinjam = v.TanggalPinjam,
                BatasWaktu = v.BatasWaktu?.ToString("dd-MM-yyyy") ?? "-",
                TanggalKembali = v.TanggalKembali,
                TelatHari = v.TelatHari ?? 0
            }).OrderBy(x => x.TanggalKembali).ToList();

            var dgv = this.Controls.Find("dataGridView1", true).FirstOrDefault() as DataGridView;
            var lblRec = this.Controls.Find("label12", true).FirstOrDefault() as Label;

            if (dgv != null)
            {
                dgv.DataSource = new SortableBindingList<LaporanPengembalianViewModel>(printData);

                if (dgv.Columns["TanggalPinjam"] != null)
                    dgv.Columns["TanggalPinjam"].DefaultCellStyle.Format = "dd MMM yyyy";

                if (dgv.Columns["TanggalKembali"] != null)
                    dgv.Columns["TanggalKembali"].DefaultCellStyle.Format = "dd MMM yyyy";
            }
        ...
        private void PrintDoc_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            g.DrawString("LAPORAN PENGEMBALIAN BARANG", new Font("Arial", 12, FontStyle.Bold), Brushes.Black, new RectangleF(margin, yPos, pageWidth, 20), formatCenter);

            var dtpAwal = this.Controls.Find("dateTimePicker2", true).FirstOrDefault() as DateTimePicker;
            var dtpAkhir = this.Controls.Find("dateTimePicker1", true).FirstOrDefault() as DateTimePicker;
            ...
            string[] headers = { "No", "No Peminjaman", "Peminjam", "Tgl Pinjam", "Batas Waktu", "Tgl Kembali", "Telat (Hari)" };
            int[] colWidths = { 30, 130, 200, 100, 100, 100, 80 };
            ...
            while (currentPrintIndex < printData.Count)
            {
                ...
                string[] rowData = {
                    ...
                    item.BatasWaktu,
                    item.TanggalKembali.ToString("dd-MM-yyyy"),
                    item.TelatHari.ToString()
                };
                ...
                for (int i = 0; i < rowData.Length; i++)
                {
                    ...
                    StringFormat fmt = (i == 0 || i == 3 || i == 4 || i == 5 || i == 6) ? formatMidCenter : formatMidLeft;
        ...
        private void BtnExportToExcel_Click(object sender, EventArgs e)
        {
            ...
            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                ...
                sfd.FileName = "Laporan_Pengembalian_Barang_" + DateTime.Now.ToString("yyyyMMdd");
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        {
                            var ws = package.Workbook.Worksheets.Add("Pengembalian");

                            ws.Cells["A1:G1"].Merge = true;
                            ws.Cells["A1"].Value = "LAPORAN PENGEMBALIAN BARANG";
                            ...
                            for (int i = 0; i < printData.Count; i++)
                            {
                                ...
                                ws.Cells[row, 3].Value = item.NamaPeminjam;
                                ws.Cells[row, 4].Value = item.TanggalPinjam.ToString("yyyy-MM-dd");
                                ws.Cells[row, 5].Value = item.BatasWaktu;
                                ws.Cells[row, 6].Value = item.TanggalKembali.ToString("yyyy-MM-dd");
                                ws.Cells[row, 7].Value = item.TelatHari;
