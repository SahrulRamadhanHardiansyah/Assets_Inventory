    ...
    public partial class LaporanPengadaanBarangUC : UserControl
    {
        ...
        private void BtnExportToExcel_Click(object sender, EventArgs e)
        {
            ...
            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                ...
                sfd.FileName = "Laporan_Pengadaan_Barang_" + DateTime.Now.ToString("yyyyMMdd");
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        {
                            var ws = package.Workbook.Worksheets.Add("Pengadaan");

                            ws.Cells["A1:I1"].Merge = true;
                            ws.Cells["A1"].Value = "LAPORAN PENGADAAN BARANG INVENTARIS";
                            ws.Cells["A1"].Style.Font.Bold = true;
                            ws.Cells["A1"].Style.Font.Size = 14;
                            ws.Cells["A1"].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;

                            string[] headers = { "No", "Tgl Pengadaan", "Nama Barang", "Kategori", "Sumber Perolehan", "Pemasok", "Jumlah", "Harga Satuan", "Sub Total" };
                            for (int i = 0; i < headers.Length; i++)
                            {
                            ...
                            for (int i = 0; i < printData.Count; i++)
                            {
                                ...
                                ws.Cells[row, 2].Value = item.Tanggal.ToString("yyyy-MM-dd");
                                ws.Cells[row, 3].Value = item.NamaBarang;
                                ws.Cells[row, 4].Value = item.Kategori;
                                ws.Cells[row, 5].Value = item.Sumber;
                                ws.Cells[row, 6].Value = item.Pemasok;
                                ws.Cells[row, 7].Value = item.Jumlah;

                                ws.Cells[row, 8].Value = item.HargaSatuan;
                                ws.Cells[row, 8].Style.Numberformat.Format = "Rp #,##0.00";

                                ws.Cells[row, 9].Value = item.SubTotal;
                            ...
                            ws.Cells[row, 1].Value = "TOTAL KESELURUHAN";
                            ws.Cells[row, 1].Style.Font.Bold = true;
                            ws.Cells[row, 1].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;

                            ws.Cells[row, 9].Value = printData.Sum(x => x.SubTotal);
                            ws.Cells[row, 9].Style.Font.Bold = true;
                            ws.Cells[row, 9].Style.Numberformat.Format = "Rp #,##0.00";

                            ws.Cells[row, 1, row, 9].Style.Border.BorderAround(ExcelBorderStyle.Thin);
