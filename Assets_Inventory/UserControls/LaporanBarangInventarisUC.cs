    ...
    public partial class LaporanBarangInventarisUC : UserControl
    {
        ...
        private void BtnExportToExcel_Click(object sender, EventArgs e)
        {
            ...
            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                ...
                sfd.FileName = "Laporan_Barang_Inventaris_" + DateTime.Now.ToString("yyyyMMdd");
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        {
                            var ws = package.Workbook.Worksheets.Add("Aset Inventaris");

                            ws.Cells["A1:I1"].Merge = true;
                            ws.Cells["A1"].Value = "LAPORAN BARANG INVENTARIS KESELURUHAN";
                            ws.Cells["A1"].Style.Font.Bold = true;
                            ws.Cells["A1"].Style.Font.Size = 14;
                            ws.Cells["A1"].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;

                            string[] headers = { "No", "Kode Inventaris", "Nama Barang", "Kategori", "Lokasi", "Ruang", "Kondisi", "Tanggal Registrasi", "Harga Satuan", "Status" };
                            for (int i = 0; i < headers.Length; i++)
                            {
                            ...
                            for (int i = 0; i < printData.Count; i++)
                            {
                                ...
                                ws.Cells[row, 5].Value = item.Lokasi;
                                ws.Cells[row, 6].Value = item.Ruang;
                                ws.Cells[row, 7].Value = item.Kondisi;

                                ws.Cells[row, 8].Value = item.TanggalRegistrasi;
                                ws.Cells[row, 8].Style.Numberformat.Format = "dd-MM-yyyy";

                                ws.Cells[row, 9].Value = item.HargaSatuan;
                                ws.Cells[row, 9].Style.Numberformat.Format = "Rp #,##0.00";

                                ws.Cells[row, 10].Value = item.Status;
