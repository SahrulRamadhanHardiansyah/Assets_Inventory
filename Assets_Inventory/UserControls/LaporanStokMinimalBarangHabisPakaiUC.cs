...
namespace Assets_Inventory
{
    public partial class LaporanStokMinimalBarangHabisPakaiUC : UserControl
    {
        ...
        private void BtnExportToExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                sfd.Filter = "Excel Files (*.xlsx)|*.xlsx"; sfd.FileName = "Laporan_StokMinimal_HabisPakai_" + DateTime.Now.ToString("yyyyMMdd");
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        {
                            var ws = package.Workbook.Worksheets.Add("Stok Minimal");
                            ws.Cells["A1:I1"].Merge = true; ws.Cells["A1"].Value = "LAPORAN STOK MINIMAL BARANG HABIS PAKAI";
