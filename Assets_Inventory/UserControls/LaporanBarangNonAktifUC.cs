...
{
    public partial class LaporanBarangNonAktifUC : UserControl
    {
        AppDbContext db = new AppDbContext();
        PrintDocument printDoc = new PrintDocument();
        List<LaporanBarangNonAktifViewModel> printData = new List<LaporanBarangNonAktifViewModel>();
        int currentPrintIndex = 0;

        public class LaporanBarangNonAktifViewModel
        {
        ...
        public LaporanBarangNonAktifUC()
        {
            InitializeComponent();
            InitializeEvents();
        }
        ...
        {
            this.Load += LaporanBarangNonAktifUC_Load;
        ...
        private void LaporanBarangNonAktifUC_Load(object sender, EventArgs e)
        {
            LoadComboBoxes();
        ...
        private void BtnTampilkanData_Click(object sender, EventArgs e)
        {
            ...
            if (idLok != 0)
            {
                ...
                query = query.Where(v => v.NamaLokasi == nmLok);
            }
            if (idRng != 0)
            {
            ...
            printData = query.ToList().Select(v => new LaporanBarangNonAktifViewModel
            {
                IdBarangNonAktif = v.IdBarangNonAktif,
            ...
            if (dgv != null)
            {
                dgv.DataSource = new SortableBindingList<LaporanBarangNonAktifViewModel>(printData);
        ...
        private void PrintDoc_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            g.DrawString("LAPORAN BARANG NON AKTIF", new Font("Arial", 12, FontStyle.Bold), Brushes.Black, new RectangleF(margin, yPos, pageWidth, 20), formatCenter);
            ...
            string[] headers = { "No", "Tgl Nonaktif", "Kode Inventaris", "Nama Barang", "Kategori", "Lokasi", "Ruang", "Jml", "Penyebab", "Keterangan" };
            int[] colWidths = { 25, 85, 110, 140, 90, 90, 90, 30, 130, 140 };
            ...
            while (currentPrintIndex < printData.Count)
            {
                ...
                string[] rowData = { (currentPrintIndex + 1).ToString(), item.TanggalNonaktif.ToString("dd-MM-yyyy"), item.KodeInventaris, item.NamaBarang, item.Kategori, item.Lokasi, item.Ruang, item.Jumlah.ToString(), item.Penyebab, item.Keterangan };
                ...
                for (int i = 0; i < rowData.Length; i++)
                {
                    ...
                    StringFormat fmt = (i == 0 || i == 1 || i == 7) ? formatMidCenter : formatMidLeft;
